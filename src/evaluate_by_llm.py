...
def _score_with_vllm(
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "vLLM is required for LLM judging but could not be imported. "
            "Install vllm (and a compatible CUDA environment), then retry."
        ) from exc

    def _construct_llm(kwargs: Dict[str, Any]) -> Any:
        try:
            return LLM(**kwargs)
        except TypeError as exc:
            message = str(exc)
            for key in ("download_dir", "gpu_memory_utilization", "max_model_len", "tensor_parallel_size"):
                if key in kwargs and (f"'{key}'" in message or key in message):
                    trimmed = dict(kwargs)
                    trimmed.pop(key, None)
                    return _construct_llm(trimmed)
            raise

    llm_kwargs: Dict[str, Any] = {
        "model": judge_model_name_or_path,
    ...
    for start in tqdm(range(0, len(prompts), batch_size), desc="LLM judging", unit="batch"):
        ...
        # vLLM returns results per prompt, but order is not strictly guaranteed; use request_id to align.
        # In practice, prompt is included, so we align by prompt string.
        by_prompt: Dict[str, str] = {}
        for out in outputs:
            text = ""
            if getattr(out, "outputs", None):
                text = str(out.outputs[0].text or "")
            by_prompt[str(getattr(out, "prompt", ""))] = text
        for prompt in batch_prompts:
            text = by_prompt.get(prompt, "")
    ...
    *,
    ...
    if not judge_model_name_or_path or not str(judge_model_name_or_path).strip():
        raise ValueError(
            ...
            "Set it to a local path or HF model id to run vLLM judging."
        )
    judge = _score_with_vllm(
        questions=questions,
